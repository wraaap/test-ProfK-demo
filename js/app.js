(() => {
    ...
    function tabs() {
        ...
        function initTabs(tabsBlock) {
            ...
            if (tabsContent.length) {
                ...
                tabsContent.forEach(((tabsContentItem, index) => {
                    ...
                    if (index == tabsActiveHash[1]) tabsTitles[index].classList.add("_tab-active");
        ...
        function setTabsStatus(tabsBlock) {
            ...
            if (tabsContent.length > 0) {
                tabsBlock.hasAttribute("data-tabs-hash");
                tabsContent = Array.from(tabsContent).filter((item => item.closest("[data-tabs]") === tabsBlock));
                tabsTitles = Array.from(tabsTitles).filter((item => item.closest("[data-tabs]") === tabsBlock));
                tabsContent.forEach(((tabsContentItem, index) => {
    ...
    function changeImgCardWidth() {
        ...
        document.querySelectorAll(".products-swiper__image").forEach((el => {
            el.setAttribute("style", "width:" + productCardWidth + "px");
        }));
    }
    window.addEventListener("resize", changeImgCardWidth);
    window.addEventListener("load", (function(e) {
        document.querySelectorAll(".products-card__item-image-ibg").forEach((el => {
            if (!el.children.length > 0) el.insertAdjacentHTML("afterbegin", '<img src="img/product-card/no-photo.jpg" alt="Нет фото">');
        }));
    }));
    isWebp();
    tabs();
